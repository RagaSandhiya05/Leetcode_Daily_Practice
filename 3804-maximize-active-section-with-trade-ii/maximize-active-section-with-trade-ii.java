...
class Solution {
    ...
    public List<Integer> maxActiveSectionsAfterTrade(String s , int queries[][]) {
        ...
        sparse.add(V);
        for(int half = 1 ; half * 2 <= nv ; half = half * 2) {
    ...
    private int rmq(int low , int high) {
        int t = 31 - Integer.numberOfLeadingZeros(high - low + 1);
        return Math.max(sparse.get(t)[low] , sparse.get(t)[high - (1 << t) + 1]);
    }
